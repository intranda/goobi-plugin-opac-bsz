...
public class BszOpacImport implements IOpacPlugin {
    private static final Logger myLogger = Logger.getLogger(BszOpacImport.class);

    private int hitcount;
    private String gattung = "Av";
        ...
        if (gattung.equalsIgnoreCase("af") || gattung.equalsIgnoreCase("Of")) {
            ...
            if (!multiVolumePpn.isEmpty()) {
                ...
                if (myOpac.getNumberOfHits(myQuery) == 1) {
                    ...
                    String parentDocumentType = getGattung(myJdomDocMultivolumeband.getRootElement().getChild("record"));
                    if (parentDocumentType != null) {
                        parentDocumentType = parentDocumentType.substring(0, 2);
                    }
                    if (parentDocumentType.equalsIgnoreCase("ab")) {
                        // set gattung in myFirstHit to av/ov
                        changeDocumentType(myFirstHit, "Av");
                    } else if (parentDocumentType.equalsIgnoreCase("Ob")) {
                        changeDocumentType(myFirstHit, "Ov");
                    }
    ...
    private void changeDocumentType(Element element, String value) {

        for (Element tempElement : element.getChildren()) {

            String feldname = tempElement.getAttributeValue("tag");
            if (feldname.equals("002@")) {
                for (Element subElement : tempElement.getChildren()) {
                    if (subElement.getAttributeValue("code").equals("0")) {
